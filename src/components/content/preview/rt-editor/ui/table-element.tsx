...
import { BorderAll, BorderBottom, BorderLeft, BorderNone, BorderRight, BorderTop } from './table-icons'
...
export function TableFloatingToolbar({ children, ...props }: React.ComponentProps<typeof PopoverContent>) {
    ...
    return (
        <Popover open={canMerge || canSplit || collapsed} modal={false}>
            ...
            <PopoverContent asChild onOpenAutoFocus={(e) => e.preventDefault()} contentEditable={false} {...props}>
                <Toolbar
                    className="scrollbar-hide bg-popover flex w-auto max-w-[80vw] flex-row overflow-x-auto rounded-md p-1 shadow-lg transition-colors duration-500 ease-out dark:shadow-black/60 print:hidden"
...
export function TableBordersDropdownMenuContent(props: React.ComponentProps<typeof DropdownMenuPrimitive.Content>) {
