...
import type { TResolvedSuggestion, TSuggestionElement, TSuggestionText } from '@udecode/plate-suggestion'

import { type NodeEntry, type Path, type TElement, ElementApi, PathApi, TextApi } from '@udecode/plate'
...
import { acceptSuggestion, getSuggestionKey, keyId2SuggestionId, rejectSuggestion } from '@udecode/plate-suggestion'
...
export const TYPE_TEXT_MAP: Record<string, (node?: TElement) => string> = {
    ...
    [ParagraphPlugin.key]: (node) => {
        if (node?.[IndentListPlugin.key] === INDENT_LIST_KEYS.todo) return 'Todo List'
        if (node?.[IndentListPlugin.key] === ListStyleType.Decimal) return 'Ordered List'
