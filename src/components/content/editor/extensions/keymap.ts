...
const enabledDefaultKeymap = defaultKeymap.filter(keybinding =>
    ...
    .map(keybinding => {
        return {
            ...keybinding,
            key: updatedDefaultKeymap.has(`key:${keybinding.key}`) ? updatedDefaultKeymap.get(`key:${keybinding.key}`) : keybinding.key,
            mac: updatedDefaultKeymap.has(`mac:${keybinding.mac}`) ? updatedDefaultKeymap.get(`mac:${keybinding.mac}`) : keybinding.mac,
            win: updatedDefaultKeymap.has(`win:${keybinding.win}`) ? updatedDefaultKeymap.get(`win:${keybinding.win}`) : keybinding.win,
            linux: updatedDefaultKeymap.has(`linux:${keybinding.linux}`) ? updatedDefaultKeymap.get(`linux:${keybinding.linux}`) : keybinding.linux
        }
    })
